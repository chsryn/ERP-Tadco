...
@section('title', 'Dashboard - ERP TADCO')
@section('page_title', 'Dashboard')
@section('page_subtitle', 'Ringkasan operasional distribusi, penjualan, stok, dan pembayaran')
...
@section('content')
    ...
    <div class="row g-3 mb-4">
        <div class="col-12 col-md-6 col-xl-3">
            <a href="{{ route('products.index') }}" class="quick-action" style="--metric-bg:#eff6ff;--metric-color:#2563eb;">
                <span class="quick-action-icon"><i class="bi bi-box-seam-fill"></i></span>
                <span>
                    <span class="d-block fw-semibold">Kelola Produk</span>
                    <small class="text-muted">{{ number_format($totalProducts, 0, ',', '.') }} produk aktif</small>
                </span>
            </a>
        </div>
        <div class="col-12 col-md-6 col-xl-3">
            <a href="{{ route('customers.index') }}" class="quick-action"
                ...
                <span>
                    ...
                    <small class="text-muted">{{ number_format($totalCustomers, 0, ',', '.') }} akun terdaftar</small>
                </span>
            </a>
        </div>
        <div class="col-12 col-md-6 col-xl-3">
        ...
        <div class="col-12 col-md-6 col-xl-3">
            <a href="{{ route('reports.index') }}" class="quick-action" style="--metric-bg:#f5f3ff;--metric-color:#6d28d9;">
            ...
            <div class="card metric-card" style="--metric-bg:#eff6ff;--metric-soft:#eff6ff;--metric-color:#2563eb;">
                ...
                <div>
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <div class="metric-value">{{ number_format((float) $totalStockAvailable, 0, ',', '.') }}</div>
        ...
        <div class="col-12 col-md-6 col-xl-4">
            <div class="card metric-card" style="--metric-bg:#fef3c7;--metric-soft:#fefce8;--metric-color:#b45309;">
                ...
                <div>
                    ...
                    <div class="d-flex align-items-center gap-2">
                        <div class="metric-value">{{ number_format($lowStockCount, 0, ',', '.') }}</div>
            ...
            <div class="card metric-card" style="--metric-bg:#fee2e2;--metric-soft:#fef2f2;--metric-color:#b91c1c;">
                ...
                <div>
                    ...
                    <div class="d-flex align-items-baseline gap-2">
                        <div class="metric-value">{{ number_format($outOfStockCount, 0, ',', '.') }}</div>
            ...
            <div class="card chart-card h-100">
                ...
                <div class="donut" style="--percent: {{ $usedPercent }};">
                    <div class="donut-inner">
                        <div class="fs-5 fw-semibold">{{ $usedPercent }}%</div>
                ...
                <div class="d-flex justify-content-between mb-2">
                    ...
                    <span>{{ number_format($totalSlots, 0, ',', '.') }}</span>
                </div>
                <div class="d-flex justify-content-between">
                    <span class="text-muted">Slot Tersedia</span>
                    <span class="text-primary">{{ number_format($availableSlots, 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
            ...
            <div class="card chart-card h-100">
                ...
                <div class="bar-chart">
                    @forelse($weeklyBars as $bar)
                        <div class="bar-day">
                            <div class="bar-pair">
                                <span class="bar inbound" style="height: {{ $bar['in_height'] ?? 0 }}%;"></span>
                                <span class="bar outbound" style="height: {{ $bar['out_height'] ?? 0 }}%;"></span>
                            </div>
                            <span class="bar-label">{{ strtoupper($bar['day']) }}</span>
                        </div>
                    @empty
        ...
        <div class="col-12 col-xl-7">
            <div class="card overflow-hidden">
                <div class="card-body border-bottom d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('delivery_orders.index') }}" class="btn btn-sm btn-dark">Lihat Semua</a>
                </div>
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            @forelse($recentDeliveryOrders as $deliveryOrder)
                                <tr>
                                    <td class="fw-semibold">{{ $deliveryOrder->do_number ?? $deliveryOrder->id }}</td>
                                    <td>{{ $deliveryOrder->customer->customer_name ?? '-' }}</td>
                                    <td>{{ optional($deliveryOrder->created_at)->format('d/m/Y') }}</td>
                                    <td><span
                                            class="badge bg-success">{{ ucfirst(str_replace('_', ' ', $deliveryOrder->status ?? 'open')) }}</span>
                                    </td>
                                </tr>
                            @empty
                                <tr>
            ...
            <div class="card overflow-hidden">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center py-2">
                        ...
                        <span class="badge bg-warning">{{ number_format($unpaidInvoices, 0, ',', '.') }}</span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center py-2">
                        <span class="text-muted">Lunas</span>
                        <span class="badge bg-success">{{ number_format($paidInvoices, 0, ',', '.') }}</span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center py-2">
                        <span class="text-muted">Jatuh tempo</span>
                        <span class="badge bg-danger">{{ number_format($overdueInvoices, 0, ',', '.') }}</span>
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <strong>Rp {{ number_format((float) $totalReceivable, 0, ',', '.') }}</strong>
                    </div>
                    <div class="d-flex justify-content-between align-items-center mt-2">
                        <span class="text-muted">Pembayaran masuk</span>
                        <strong class="text-primary">Rp {{ number_format((float) $totalPayments, 0, ',', '.') }}</strong>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-12 col-xl-6">
            <div class="card overflow-hidden">
                <div class="card-body border-bottom d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('invoices.index') }}" class="btn btn-sm btn-dark">Lihat Semua</a>
                </div>
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            @forelse($recentInvoices as $invoice)
                                <tr>
                                    <td class="fw-semibold">{{ $invoice->invoice_number ?? $invoice->id }}</td>
                                    <td>{{ $invoice->customer->customer_name ?? '-' }}</td>
                                    <td>Rp {{ number_format((float) ($invoice->grand_total ?? 0), 0, ',', '.') }}</td>
                                    <td><span
                                            class="badge bg-{{ $invoice->status === 'paid' ? 'success' : 'warning' }}">{{ ucfirst(str_replace('_', ' ', $invoice->status ?? 'unpaid')) }}</span>
                                    </td>
                                </tr>
                            @empty
                                <tr>
        ...
        <div class="col-12 col-xl-6">
            <div class="card overflow-hidden">
                <div class="card-body border-bottom d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('payments.index') }}" class="btn btn-sm btn-dark">Lihat Semua</a>
                </div>
                    ...
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            @forelse($recentPayments as $payment)
                                <tr>
                                    <td class="fw-semibold">{{ $payment->payment_number ?? $payment->id }}</td>
